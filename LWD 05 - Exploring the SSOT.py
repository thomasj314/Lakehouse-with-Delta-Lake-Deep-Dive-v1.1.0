...
# MAGIC 
# MAGIC Recall that we will need to tell Spark that our format is a Delta table, which we can do with our **`format()`** method. 
# MAGIC 
# MAGIC Additionally, instead of passing in the path
# MAGIC as we did in previous notebooks, we need to pass in the health tracker variable **`DA.paths.processed`**.
# MAGIC 
...
# MAGIC Let’s run a query to discover the timing of the missing records.
# MAGIC 
# MAGIC We use a Databricks visualization to display the number of records per day.
# MAGIC 
# MAGIC It appears that we have no records for device 4 for the last few days of the month.
...
# MAGIC Upon our initial load of data into the **health_tracker_processed** table, we noted that there are broken records in the data. 
# MAGIC 
# MAGIC In particular, we made a note of the fact that several negative readings were present even though it is impossible to record a negative heart rate.
# MAGIC 
